...
def get_binary(target_grey, thresh=200, maxval=255):
    ...
    (_, target_binary) = cv2.threshold(src=blur_target,
                                       thresh=thresh,
                                       maxval=maxval,
...
def get_most_similar(df_matchDist, threshold=0.1):
    ...
    close_contour_sets = {a: list(set(df_reduced.iloc[a, :].dropna(
    ).index.values)-set([a])) for a in df_reduced.index}
...
def get_similar_bands(contours, dirname, target_original, target_grey, skip_first=True, colored=False):
    ...
    for idx, c in enumerate(contours[skip_first:]):
        ...
        if y - 5 < 0:
            ...
            stretched_y = y - 5
        if colored:
            band_images[idx] = target_original[stretched_y: y +
                                               h + 5, x: x + w, :]  # for colored images
        else:
            band_images[idx] = target_grey[stretched_y: y +
                                           h + 5, x: x + w]  # add 5px to y axis each way
...
def plot_colored_bands(sorted_idx, band_images, dirname):
    ...
    if dirname:
        plt.savefig("{}/3-band_lineup{}.png".format(dirname, '-'.join(str(idx)
                                                                      for idx in idx_filtered)), bbox_inches='tight')
...
def analyse_image(target_fn, dirname, binmin=100, binmax=255, contour_color=(255, 0, 0), skip_first=True,
    ...
    (contours, hierarchy) = cv2.findContours(
        target_binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2:]
    print(f"{len(contours)} contours found")

    # annotate contours
    band_cts = collect_contours(contours,
                                hierarchy,
                                minarea=minarea,
                                maxarea=maxarea,
    ...
    # find similar bands
    band_images, close_contour_sets = get_similar_bands(band_cts,
                                                        dirname,
                                                        target_original,
                                                        target_grey,
                                                        skip_first=skip_first,
...
def find_discontinuities(target_grey, ksize=(3, 13), min_edge_threshold=15, max_edge_threshold=100, min_length=30, dirname=""):
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi/180,
                            threshold, min_length, maxLineGap)
    ...
    # save image
    plt.savefig("{}/1-discontinuity_detection.png".format(dirname),
                bbox_inches='tight')
